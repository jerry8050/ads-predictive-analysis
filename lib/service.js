...
function addStats(items, stats = {}) {
    ...
    if (_.isArray(items)) {
        items.forEach(item => {
            ...
            item["click_through_rate"] = item.total_clicks && stats.totalImpressions ? parseFloat(((item.total_clicks / stats.totalImpressions) ).toFixed(6)) : 0;
            ...
            for (let key in item.industries) {
                ...
                ind["click_through_rate"] = ind.total_clicks && ind.total_impressions ? parseFloat(((ind.total_clicks / ind.total_impressions) ).toFixed(6)) : 0;

                // ind["click_percentage"]      = ind.total_clicks && byIndustry[key].total_clicks ? parseFloat(((ind.total_clicks / byIndustry[key].total_clicks * 100).toFixed(6))) : 0;
        ...
        items.forEach(item => {
            ...
            for (let key in item.industries) {
                ...
                let avgCtr       = byIndustry[key].total_clicks / byIndustry[key].total_impressions;
                ind["index"]     = parseFloat(((ind.click_through_rate / avgCtr) * 100).toFixed(6));
                let stdDeviation = Math.sqrt(Math.pow(ind.click_through_rate - avgCtr, 2) / byIndustry[key].count);
                ind["z_score"]   = parseFloat(((ind.click_through_rate - avgCtr) / stdDeviation).toFixed(6));
                totalClickThroughRate += ind["click_through_rate"];
            }
        });
